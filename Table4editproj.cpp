...
void __fastcall TEdit_proj_instrums::Grid1Form()
{
   //	Label_error->Caption = IntToStr(Proj_editor_w->row_id);
	...
	mysql_real_connect(db, "zaoios.ru","rt_2018","rt2_2018", "rt_rescalc", 0, NULL, 0);
	AnsiString query = "SELECT tl_id, tl_artic, tl_outgo FROM rt_proj_tool WHERE tl_id = " +String(Proj_editor_w->row_id)+ "";
	mysql_query(db, query.c_str());    //ansi string v char
	MYSQL_RES *result = mysql_store_result(db);
	MYSQL_ROW row ;
	char *table_name[]  = {"ID проекта", "Артикул", "Расход"};
	for (i = 0; i < 3; i++) {
		edit_proj_grid->Cells[i][0] = table_name[i];
	}
	if (result) {
		for (i=1; (row = mysql_fetch_row(result))!= 0; i++) {
			for (k=0;k<=3; k++)  {   //пока есть
   ...
   mysql_close(db);
   mysql_server_end();
   //Edit_proj_instrums->edit_proj_grid->Refresh();
...
void __fastcall TEdit_proj_instrums::GridUpdateForm()
{
	...
	MYSQL *db = mysql_init(NULL);                                                          //IntToStr(Proj_editor_w->t4_id)
	mysql_real_connect(db, "zaoios.ru","rt_2018","rt2_2018", "rt_rescalc", 0, NULL, 0);
	AnsiString query = "INSERT INTO rt_proj_tool (tl_id, tl_artic,tl_outgo) VALUES ("+String(Proj_editor_w->row_id)+","
		""+ String(Edit_proj_instrums->col_artic) +","+ T4addtrack1->t4_tck +")";
...
void __fastcall TEdit_proj_instrums::t4_butt_delClick(TObject *Sender)
{
   ...
   Edit_proj_instrums->edit_proj_grid->Rows[Edit_proj_instrums->row_clean_id+1]->Clear();
