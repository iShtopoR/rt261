...
{
   //	   Label6->Caption = IntToStr(T4addtrack1->t4_tck);
	mysql_server_init(0, NULL, NULL);
	int i, k;
	edit_proj_grid->RowCount = 1;
	MYSQL *db = mysql_init(NULL);                                                          //IntToStr(Proj_editor_w->t4_id)
	mysql_real_connect(db, "zaoios.ru","rt_2018","rt2_2018", "rt_rescalc", 0, NULL, 0);
	AnsiString query = "SELECT tl_id, tl_artic, tl_outgo, tl_info FROM rt_proj_tool"; //WHERE tl_id = "+IntToStr(row_id)+"";
	mysql_query(db, query.c_str());    //ansi string v char
	MYSQL_RES *result = mysql_store_result(db);
	MYSQL_ROW row ;
	char *table_name[]  = {"ID", "Артикул", "Расход", "Прим."};
	for (i = 0; i < 4; i++) {
...
void __fastcall TEdit_proj_instrums::GridUpdateForm()
{
   /*	mysql_server_init(0, NULL, NULL);
	MYSQL *db = mysql_init(NULL);                                                          //IntToStr(Proj_editor_w->t4_id)
	mysql_real_connect(db, "zaoios.ru","rt_2018","rt2_2018", "rt_rescalc", 0, NULL, 0);
	query = "INSERT INTO rt_proj_tool (tl_artic,tl_outgo,tl_info) VALUES ("+ article->Text +","
